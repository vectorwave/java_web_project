...
prefix="form"%> <%@taglib uri="http://java.sun.com/jsp/jstl/fmt" prefix="fmt"%>
...
<html>
  ...
  <body onload="onRuleLoaded()">
    <br /><br /><br /><br />
    <!-- Start Header Area -->
    <div style="background-image: url('image/about_banner.jpg')">
      <h1 style="color: black; text-align: center">智能客服</h1>
      ...
      <div>
        ...
        <input
          style="position: relative; left: 30%; border: #090909 solid"
          id="say"
          name="say"
          type="text"
          value=""
          size="80"
          onkeydown="keyin(event)"
        />
        <!-- 按 enter 時呼叫 keyin() 回答 -->
        <input type="submit" value="送出" onclick="say()" id="clear" />
        <!-- 當送出按鈕按下，就呼叫 say() 函數回答 -->
      </div>
      ...
      >
        <div>
          ...
          <p id="ty" style="font-size: 20px">
            Hi!請告訴我你/妳的問題(景點推薦&旅程問題&金流問題&其他問題)
          </p>
        </div>
        <span id="msg"></span>
      </div>
...
Eliza 就是一個這麼簡單的程式而已。*/
      ...
      var qaList = [
        ...
        { Q: "景點|推薦景點", A: "想去北部、中部、南部、東部呢?" },
        {
          Q: "北部",
          A: "這是北部旅遊連結⬇️<br><p id='tyu'><a href='http://localhost:8081/jotravel/front/productPage?tag=%E5%8C%97%E9%83%A8'>http://localhost:8081/jotravel/front/productPage?key=%E5%8C%97%E9%83%A8</a>",
        },
        {
          Q: "中部",
          A: "這是中部旅遊連結<p id='tyu'><a href='http://localhost:8333/Nice_eCommerce/FrontPageSearchBySubCategory?name=%E4%BC%91%E9%96%92%E9%9E%8B'>http://localhost:8333/Nice_eCommerce/FrontPageSearchBySubCategory?name=%E4%BC%91%E9%96%92%E9%9E%8B</a>",
        },
        {
          Q: "東部",
          A: "這是東部旅遊連結<p id='tyu'><a href='http://localhost:8333/Nice_eCommerce/FrontPageSearchBySubCategory?name=%E4%BC%91%E9%96%92%E9%9E%8B'>http://localhost:8333/Nice_eCommerce/FrontPageSearchBySubCategory?name=%E4%BC%91%E9%96%92%E9%9E%8B</a>",
        },

        {
          Q: "墾丁",
          A: "這是墾丁旅遊連結⬇️<br><p id='tyu'><a href='http://localhost:8081/jotravel/front/productPage?key=%E5%A2%BE%E4%B8%81'>http://localhost:8081/jotravel/front/productPage?key=%E5%A2%BE%E4%B8%81</a>",
        },
        ...
        {
          Q: "一般退貨",
          A: "對於在阿迪達斯官方購物網站所購買的商品，您可以在收到商品(以物流單據上的簽收日期為準)次日起的7天商品鑑賞期內申請一般退貨，尚無提供換貨服務。一般退貨條件適用於台灣阿迪達斯官方購物網站，包含一般商品、特價及促銷商品，但不包含使用優惠券訂單，優惠券退貨規則請參考「2. 優惠券活動商品退貨」。*請注意，您所申請的退貨，需符合以下所述的「3. 退貨條件」。",
        },
        {
          Q: "優惠券活動商品退貨",
          A: "對於在阿迪達斯官方購物網站所購買的商品，若您於結帳時使用優惠券折扣代碼，該筆訂單僅能接受整筆訂單退貨。意即同筆訂單內，即使該商品未享有優惠券折扣，仍無法分別退貨。*請注意，您所申請的退貨，需符合以下所述的「3. 退貨條件」。",
        },
        {
          Q: "退貨條件",
          A: "商品在7天鑑賞期內。依消保法規定，7天鑑賞期指收到商品(以物流單據上的簽收日期為準)次日起的7天內，並包含例假日。第7天為假日時，末日順延至次日。商品保持完好、未經使用、洗滌或修改、無沾染污漬或化妝品等痕跡，且不影響二次銷售。商品的外包裝完好無損（包括外包裝箱或外包裝袋，請不要直接在我們的鞋盒上面黏貼膠帶，請在鞋盒外加盒子和袋子），商品附件、說明書、吊牌、標識、標籤等齊全。*提醒您，鞋類商品的鞋盒也屬於商品，退貨時，鞋盒與鞋子需一併退回。如您所購買的商品有附加的贈品，請將贈品一併退回，贈品等同於一般商品，請保持贈品的品質與完整性。*提醒您，若贈品不能退回，恕無法接受退貨。",
        },

        { Q: "我想客訴|我要客訴", A: "這是客服表單連結⬇️<br><p id='ty'><a href='http://localhost:8081/jotravel/message/form'>http://localhost:8081/jotravel/message/form</a>" },
      ...
      $(function () {
        t1 = setInterval("showAuto1()", 13);
        t = setInterval("showAuto()", 12);
      });
      ...
      function say() {
        // 當送出鍵按下時，會呼叫這個函數進行回答動作
        append(
          "<div  id='tyu' style=''><img src='image/20.jpg'><p style='position:relative;top:20px;'>" +
      ...
      function answer() {
        // 回答問題
        setTimeout(function () {
          // 停頓 1 到 3 秒再回答問題 (因為若回答太快就不像人了，人打字需要時間)
          append(
            "<div><img src='image/16.jpg'><p id='ty'>" +
              getAnswer() +
              "</p></div>"
          );
        }, 1000 + random(1000));
        ...
        for (var i in qaList) {
          ...
          try {
            ...
            for (var qi in qList) {
              ...
              var q = qList[qi];
              if (q == "")
                // 如果是最後一個「空字串」的話，那就不用比對，直接任選一個回答。
